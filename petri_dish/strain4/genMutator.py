...
def makeBetter(mutableCode):
    ...
    # So, for now let's just increase the amount by one...~?
...
def mutate(fName):
    ...
    if os.path.exists(fName):
        print("DNA FILE FOUND: " , fName)

        """
            The DNA file provided gave good results, so let's try to enhance it.
            First read code in newCodeFile.txt, and then read DNA file.
            Extract mutation (mutable) code.
            Compare code? Doesn't matter, the DNA file is what worked best, so attempt to make it better.
        ...
        print("{} DNA FILE NOT FOUND!".format(fName))
